...
void pin_writer (uint8_t count1, uint8_t count2, uint8_t count3) {
	...
	DIO_WritePort(1, b , STD_LOW);  //output pins for 1st 7 segmant [port A]
	DIO_WritePort(3, d , STD_LOW);  //output pins for 2nd 7 segmant [port B]
	DIO_WritePort(4, e , STD_LOW);  //output pins for 3rd 7 segmant [port D]
}
...
void initial() {
	...
	Port_SetPinDirection (1, 0x30, PORT_PIN_IN);	
	Port_SetPinDirection (0, 0x40, PORT_PIN_IN);
	
	Port_SetPinPullUp (1, 0x30, 1);
	Port_SetPinPullUp (0, 0x40, 1);
	...
	int main()
	{
		...
		if (reset == 0){
				...
				count_2 = 0;
				count_3 = 0;
				while (reset == 0){reset = DIO_ReadPort(0, 0x40);delay(20);}
				pin_writer (count_1, count_2, count_3);	
			}
